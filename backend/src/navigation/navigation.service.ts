    ...
    try {
      const crawler = new PlaywrightCrawler({
        ...
        requestHandler: async ({ page, log }) => {
          ...
          await page.waitForSelector('.site-nav__item', { timeout: 10000 }).catch(() => null);
          ...
          const menuData = await page.evaluate(() => {
            ...
            document.querySelectorAll('.site-nav__item').forEach((navItem) => {
              ...
              if (title && slug) {
                const subCats: any[] = [];
                navItem.querySelectorAll('.site-nav__dropdown-link').forEach((sub) => {
                  subCats.push({
          ...
          for (const item of menuData) {
            ...
            let nav = await this.navRepository.findOne({ where: { slug: item.slug } });
            if (!nav) {
              nav = this.navRepository.create({ title: item.title, slug: item.slug });
